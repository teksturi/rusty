...
use std::{
    ops::Deref,
    path::{Path, PathBuf}, cell::RefCell, borrow::BorrowMut,
};
...
use crate::{
    ...
    DebugLevel, FormatOption, OptimizationLevel, Target,
};
...
use inkwell::{
    context::Context,
    execution_engine::{JitFunction, ExecutionEngine},
    types::BasicType,
};
use inkwell::{
    module::Module,
...
impl<'ink> CodeGen<'ink> {
    ...
    pub fn generate_llvm_index(
        ...
        let llvm_type_index =
            data_type_generator::generate_data_types(&llvm, &mut self.debug, global_index, annotations)?;
        ...
        //TODO: Should these be by module?
...
impl<'ink> GeneratedModule<'ink> {
    ...
    pub fn persist(
        ...
        match format {
            FormatOption::Object | FormatOption::Static | FormatOption::Relocatable => {
                self.persist_as_static_obj(output_dir, output_name, target, optimization_level)
            }
            FormatOption::Shared => {
                self.persist_to_shared_pic_object(output_dir, output_name, target, optimization_level)
            }
            FormatOption::Bitcode => self.persist_to_bitcode(output_dir, output_name),
            FormatOption::IR => self.persist_to_ir(output_dir, output_name),
        }
    }
    ...
    pub fn persist_as_static_obj(
        ...
        self.persist_to_obj(output_dir, output_name, RelocMode::Default, target, optimization_level)
    }
    ...
    pub fn persist_to_shared_pic_object(
        ...
        self.persist_to_obj(output_dir, output_name, RelocMode::PIC, target, optimization_level)
    }
    ...
    pub fn persist_to_shared_object(
        ...
        self.persist_to_obj(output_dir, output_name, RelocMode::DynamicNoPic, target, optimization_level)
    }
    ...
    /// * `output` - the location on disk to save the output
    pub fn persist_to_bitcode(&self, output_dir: &Path, output_name: &str) -> Result<PathBuf, Diagnostic> {
    ...
    pub fn persist_to_ir(&self, output_dir: &Path, output_name: &str) -> Result<PathBuf, Diagnostic> {
        let output = output_dir.join(output_name);
        self.module
    ...
    pub fn add_global_mapping(&self, function_name: &str, local_function: usize) {
        let engine = self.get_execution_engine();
