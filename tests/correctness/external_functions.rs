...
fn test_external_function_called() {
    ...
    let source = SourceCode { path: "external_test.st".into(), source: prog.to_string() };
    let context = CodegenContext::new();
    let module = compile(&context, source);
    module.add_global_mapping("times_two", times_two as usize);
...
fn sized_variadic_call() {
    ...
    Target::initialize_native(&InitializationConfig::default()).unwrap();
    let source = SourceCode { path: "external_test.st".into(), source: src.to_string() };
    let context = CodegenContext::new();
    let module = compile(&context, source);
    module.add_global_mapping("add", add as usize);
...
fn sized_pointer_variadic_call() {
    ...
    let context = CodegenContext::new();
    let module = compile(&context, source);
    module.add_global_mapping("add_ref", add_ref as usize);
...
fn string_sized_variadic_call() {
    ...
    let context = CodegenContext::new();
    let module = compile(&context, source);
    module.add_global_mapping("verify_string", verify_string as usize);
...
fn string_sized_pointer_variadic_call() {
    ...
    let context = CodegenContext::new();
    let module = compile(&context, source);
    module.add_global_mapping("verify_string_ref", verify_string_ref as usize);
